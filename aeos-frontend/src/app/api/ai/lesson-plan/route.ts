...
export async function POST(req: Request) {
  try {
    ...
    const response = await fetch(`${jarvisUrl}/api/v1/planning/generate`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(`AI Core Planning Error: ${errorText}`);
    }

    const data = await response.json();
    return NextResponse.json(data);
  } catch (error: any) {
    console.error("Error in AI planning proxy:", error);
