...
export async function POST(req: Request) {
  try {
    ...
    const response = await fetch(`${jarvisUrl}/api/v1/communication/generate-parent-report`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(`AI Core Communication Error: ${errorText}`);
    }

    const data = await response.json();
    return NextResponse.json(data);
  } catch (error: any) {
    console.error("Error in AI communication proxy:", error);
